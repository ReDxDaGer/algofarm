...
#[pyfunction]
fn quicksort(py: Python<'_>, mut arr: Vec<i64>) -> Vec<i64> {
    py.allow_threads(|| {
...
#[pyfunction]
fn quicksort_f64(py: Python<'_>, mut arr: Vec<f64>) -> Vec<f64> {
    py.allow_threads(|| {
...
#[pyfunction]
fn mergesort(py: Python<'_>, mut arr: Vec<i64>) -> Vec<i64> {
    py.allow_threads(|| {
        if arr.len() <= 1 {
            return arr;
        }
...
#[pyfunction]
fn parallel_sort(py: Python<'_>, mut arr: Vec<i64>) -> Vec<i64> {
    py.allow_threads(|| {
        let n = arr.len();
        if n < 50_000 {
            quicksort_worker(&mut arr);
...
#[pyfunction]
fn kth_smallest(py: Python<'_>, mut arr: Vec<i64>, k: usize) -> PyResult<i64> {
    if k == 0 || k > arr.len() {
...
#[pyfunction]
fn top_k(py: Python<'_>, mut arr: Vec<i64>, k: usize) -> Vec<i64> {
    py.allow_threads(|| {
        let k = k.min(arr.len());
        if k == 0 {
            return Vec::new();
...
#[pyfunction]
fn dedup(py: Python<'_>, arr: Vec<i64>) -> Vec<i64> {
    py.allow_threads(|| {
        let mut cap = 16usize;
        while cap < arr.len() * 2 {
            cap *= 2;
...
#[pyfunction]
fn sieve(py: Python<'_>, limit: u64) -> Vec<u64> {
    py.allow_threads(|| {
        if limit < 2 {
            return Vec::new();
        }
...
fn algofarm(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(quicksort_f64, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(kth_smallest, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(mod_pow, m)?)?;
